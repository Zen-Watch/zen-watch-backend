...
export async function create_ifttt_action_definition_logic(payload: any) {
  try {
    ...
    const result: any = await pool!.query(`insert into ifttt_action_definition (dev_id, categ_id, is_public, is_trusted_source, is_compute_intensive, target_resource_name, action_name, action_description, action_signature, action_code, action_expected_output, action_expected_output_description) values (? ,? ,? ,? ,? ,? ,? ,? ,? ,? ,? ,?);`, [dev_id, categ_id, is_public, is_trusted_source, is_compute_intensive, target_resource_name, action_name, action_description, action_signature, action_code, action_expected_output, action_expected_output_description]);
    return result[0];
  } catch (e) {
    throw e;
  }
