...
export async function fetch_event_evm_transaction_insights(email: string, chains: string[], lookback_period: number) {
  try {
    ...
    const successful_txn_metrics = await get_metrics_for_evm_transactions(dev.id, TXN_STATUS_SUCCESS, chain_events, lookback_period);
    const error_txn_metrics = await get_metrics_for_evm_transactions(dev.id, TXN_STATUS_ERROR, chain_events, lookback_period);
    const result = {
