...
import { get_app_exchange_currency, get_evm_chain_event_names } from "../utils/util.methods";
...
export async function fetch_event_evm_transaction_gas_cost_insights(email: string, chains: string[], lookback_period: number, exchange_currency: string) {
  try {
    ...
    const app_exchange_currency = get_app_exchange_currency(exchange_currency)
