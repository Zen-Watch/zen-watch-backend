import { create_ifttt_action_definition_logic, fetch_action_definition_details_logic, fetch_ifttt_public_approved_action_definition_code_logic, fetch_ifttt_public_approved_action_definitions_logic, fetch_submitted_ifttt_action_definitions_logic, fetch_unique_ifttt_target_resource_names_for_public_actions_logic, update_ifttt_action_definition_approval_status_logic, update_ifttt_action_definition_code_info_logic } from "../logic/ifttt_action_definition.logic";
...
export async function fetch_ifttt_public_approved_action_definitions(target_resource_name: string, email: string) {
    try {
        const dev = await get_developer_by_email_from_cache(email);
        const rows = await fetch_ifttt_public_approved_action_definitions_logic(target_resource_name, dev.id);
        return { status: STATUS_OK, message: rows }
    } catch (error) {
...
export async function fetch_ifttt_public_approved_action_definition_code(action_id: number, email: string) {
    try {
        const dev = await get_developer_by_email_from_cache(email);
        const rows = await fetch_ifttt_public_approved_action_definition_code_logic(action_id, dev.id);
        return { status: STATUS_OK, message: rows }
    } catch (error) {
        return { status: STATUS_NOT_FOUND, message: 'Error during action definition code explanation fetch. Please contact support@zen.watch' }
    }
}
