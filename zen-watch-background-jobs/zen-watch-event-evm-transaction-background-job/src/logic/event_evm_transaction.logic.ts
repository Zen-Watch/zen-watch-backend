...
export async function fetchUnprocessedEVMTransactionEvents() {
    ...
    const result: any = await pool.query(`select * from event_evm_transaction where status=? and event_type in (?, ?)`, [UNPROCESSED_ENTITY, POLYGON_MAINNET_TRANSACTION_EVENT_TYPE, ETHEREUM_MAINNET_TRANSACTION_EVENT_TYPE]);
    return result[0];
}
...
export async function saveBackFillJson(event:any, backfill_json_str:string) {
    ...
    try{
        const pool = await connect_to_mysql();
        const result:any = await pool.query(`UPDATE event_evm_transaction SET backfill_json= ?, status= ? where id = ?`, [backfill_json_str, PROCESSED_ENTITY, event.id]);
    }catch(e){
