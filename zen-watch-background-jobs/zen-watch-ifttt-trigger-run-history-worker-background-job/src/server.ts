...
import { handle_fetch_unprocessed_ifttt_trigger_run_history_events, handle_process_ifttt_trigger_run_history_events } from './handlers/ifttt_trigger_run_history.handler';
...
function run_task() {
    try {
        console.log('Run_task next loop for ifttt trigger run history worker background job started at -', dayjs().format());

        // read all unprocessed IFTTT trigger run history events
        handle_fetch_unprocessed_ifttt_trigger_run_history_events()
            .then(ifttt_trigger_run_history_events => {
                ...
                handle_process_ifttt_trigger_run_history_events(ifttt_trigger_run_history_events)
                    .then(() => {
        ...
        console.log('Process terminated with error in ifttt trigger run history worker run_task', e);
    } finally {
        console.log('Run_task next loop for ifttt trigger run worker background job finished at -', dayjs().format());
        console.log('------------------------------------');
