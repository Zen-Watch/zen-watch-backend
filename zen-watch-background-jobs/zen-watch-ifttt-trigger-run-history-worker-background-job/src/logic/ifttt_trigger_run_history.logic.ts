...
import { IFTTT_TRIGGER_RUN_HISTORY_WORKER_STATUS_UNDER_PROCESSING, IFTTT_TRIGGER_RUN_HISTORY_WORKER_STATUS_UNPROCESSED } from "../utils/constants";
dotenv.config();
...
export async function fetch_unprocessed_ifttt_trigger_run_history_events() {
    ...
    const select_result: any = await pool!.query(`select * from ifttt_trigger_run_history where ifttt_trigger_run_history_worker_shard_id=? and trigger_run_status=? FOR UPDATE;`, [ifttt_trigger_run_history_worker_id, IFTTT_TRIGGER_RUN_HISTORY_WORKER_STATUS_UNPROCESSED]);
    ...
    for (let _row of select_result[0]) {
        await pool!.query(`update ifttt_trigger_run_history set trigger_run_status=? where id=?;`, [IFTTT_TRIGGER_RUN_HISTORY_WORKER_STATUS_UNDER_PROCESSING, _row.id]);
    }    
